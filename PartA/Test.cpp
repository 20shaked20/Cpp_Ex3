...
std ::vector<double> vec7 = {-1,-1,-1,-1,-1,-1,-1,-1,-1}; /*z*/
std ::vector<double> vec8 = {1,1,1,1,1,1,1,1,1}; /*x*/
Matrix a{vec1,3,3};
Matrix b{vec2,3,3};
Matrix c{vec3,3,3};
Matrix d{vec4,3,3};
...
TEST_CASE("Negative Input") {
    ...
    CHECK_THROWS(Matrix mat(vec1, -2,-3));
    CHECK_THROWS(Matrix mat(vec1, -5,-3));
    CHECK_THROWS(Matrix mat(vec1, -6,-3));
